...
wattpadApp.sortInfo = function() {
    ...
    $('input:radio').change(function(){
        var numberCheckRadio = $('input[type=radio]:checked').length
        
        if (numberCheckRadio == 3) {
            $('form').on('submit', function(e) {
                ...
                if(questionOneInput == 22 && questionTwoInput == 5 || questionTwoInput == 5 && questionThreeInput == 5 || questionOneInput == 22 && questionThreeInput == 5) {
                    ...
                    $.ajax({
                        ...
                        var scifiDescs = [
                            ...
                            $('<p>').text("Science fiction fanatics may say that such stories are complete rational and scientifically established, but last time I checked, humans haven't colonized Mars yet! Are you irritated yet?! Fret not, read your story and you'll be back to being as calm, collected, and shiny as a brand-new robot.")
                        ];
                        ...
                        var $storyName = randomscifiStory.title;

                        var $storyAuthor = randomscifiStory.user;

                        $('.results-text h5').append($storyName + ' by ' + $storyAuthor);

                        $('.results-text').append(randomScifiDesc);

                        //Container for story cover, title, desc, etc.
                        var $resultsTitle = $('<a>').attr({
                            href: randomscifiStory.url,
                            target: "_blank"
                        });
                        var $resultsLink = $('<h3>').text(randomscifiStory.title);
                        var $resultsLinkOpen = $('<i>').addClass('fa fa-external-link')

                        $resultsLink.append($resultsLinkOpen);
                        $resultsTitle.append($resultsLink);

                        var $resultsCover = $('<img>').attr({
                            src: randomscifiStory.cover,
                            alt: "Cover of " + randomscifiStory.title
                        });
                        var $resultsDesc = $('<p>').text(randomscifiStory.description);

                        $('.results-story').append($resultsTitle, $resultsCover, $resultsDesc);

                        //Heading for "You are..."
                        $('.you-are').append("You are...");

                        //Container for buttons 

                        //Button for restart
                        $('.button-again').append('Give me another go!');
                    });
                }
                ...
                //If statement for the Vampire category input
                    ...
                    $.ajax({
                        ...
                        var $storyName = randomVampireStory.title;

                        var $storyAuthor = randomVampireStory.user;

                        $('.results-text h5').append($storyName + ' by ' + $storyAuthor);

                        $('.results-text').append(randomVampireDesc);
                ...
                //If statement for the Romance category
                else if (questionOneInput == 4 && questionThreeInput == 4 || questionTwoInput == 5 && questionThreeInput == 4 || questionTwoInput == 5 && questionThreeInput == 2 || questionOneInput == 4 && questionTwoInput == 18) {
                    ...
                    $.ajax({
                        ...
                        //Container for text
                        var $storyName = randomRomanceStory.title;

                        var $storyAuthor = randomRomanceStory.user;

                        $('.results-text h5').append($storyName + ' by ' + $storyAuthor);

                        $('.results-text').append(randomRomanceDesc);
                ...
                //If the user selects they're cheesy, the genre is automatically "Poetry"
                    ...
                    $.ajax({
                        ...
                        //Container for text
                        var $storyName = randomPoetryStory.title;

                        var $storyAuthor = randomPoetryStory.user;

                        $('.results-text h5').append($storyName + ' by ' + $storyAuthor);

                        $('.results-text').append(randomPoetryDesc);
...
$(function() {
    ...
    //Scrolling to next option on click of selection
    if ($(window).width() > 940) {
